...
// This was included as a port of back-porting a fix for issue #1202.
// The original used `scheduled_actor::default_exception_handler` but in the
// current version `ptr` has a stricter type, so we add a compatible copy here.
error default_exception_handler(local_actor* ptr, std::exception_ptr& x) {
...
public:
  ...
  resumable::resume_result resume(execution_unit* ctx, size_t) override {
    ...
    try {
      ...
      rsn = default_exception_handler(self_, ptr);
    }
    try {
      self_->on_exit();
    } catch (...) {
